...
# See if the backup is still running. If this script is already running then abort. 
# This is necessary if you are running a cron job and want only one instance of this script at a time.
# You may delete it if you don't care for this check. It is important to name the script file 'backup.bash' for this check to work.
export pidof_out=`/bin/pidof -x /usr/local/lib/pressupbox/mirror.sh`
num=`echo $pidof_out | wc -w`
if [ "$num" != "1" ]
then
 echo "/usr/local/lib/pressupbox/mirror.sh is already running"
 exit
fi
echo /usr/local/lib/pressupbox/mirror.sh is not running
